...
final class OpenAIResponsesShared
{
    ...
    public static function processStream(iterable $events, AssistantMessageEventStream $stream, Model $model): AssistantMessage
    {
        ...
        $output = self::createOutput($model);
        ...
        foreach ($events as $event) {
            switch ($event['type'] ?? null) {
                ...
                case 'response.output_item.done':
                    $item = $event['item'];
                    $index = array_key_last($content);
                    if (($item['type'] ?? null) === 'reasoning' && $content[$index] instanceof ThinkingContent) {
                    ...
                    } elseif (($item['type'] ?? null) === 'message' && $content[$index] instanceof TextContent) {
                    ...
                    } elseif (($item['type'] ?? null) === 'function_call' && $content[$index] instanceof ToolCall) {
        ...
        $output = self::snapshot($model, $content, $usage, $stopReason, $responseId);
